...
@export var house_min_elevation = 0.0 :
	set(value):
		house_min_elevation = value
		build_houses()
@export var house_max_elevation = 1.0 :
	set(value):
		house_max_elevation = value
		build_houses()
...
var vert_array: PackedVector3Array
...
func build_houses():
	...
	for vidx in range( vert_array.size() ) :
		var point = vert_array[vidx].normalized()
		var value = 1.5 + pow(abs(noise.get_noise_3d(point.x * 100.0, point.y * 100.0, point.z * 100.0)), amplitude_pow) * amplitude
...
func build():
	...
	var mesh_arrays : Array = get_mesh_arrays()
	vert_array = mesh_arrays[Mesh.ARRAY_VERTEX]
	...
	for vidx in range( vert_array.size() ) :
		...
		var value = 1.5 + pow(abs(noise.get_noise_3d(vert_array[vidx].x * 100.0, vert_array[vidx].y * 100.0, vert_array[vidx].z * 100.0)), amplitude_pow) * amplitude
...
func create_house(pos: Vector3, value):
	...
	transform.basis = transform.basis * Basis().scaled(Vector3(
		randf_range(0.5, 1.0) / value,
		randf_range(0.5, 1.0) / value,
		randf_range(0.5, 3.0) / value,
	))
